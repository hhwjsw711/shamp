...
import { internalAction, mutation, query, internalQuery, } from "./_generated/server.js";
import { Workpool } from "@convex-dev/workpool";
import { RateLimiter } from "@convex-dev/rate-limiter";
import { api, components, internal } from "./_generated/api.js";
import { internalMutation } from "./_generated/server.js";
import { vEmailEvent, vOptions, vStatus, } from "./shared.js";
...
export const sendEmail = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.options.testMode && !isValidResendTestEmail(args.to)) {
            throw new Error(`Test mode is enabled, but email address is not a valid resend test address. Did you want to set testMode: false in your ResendOptions?`);
        }
...
export const updateManualEmail = mutation({
    ...
    returns: v.null(),
    handler: async (ctx, args) => {
        const finalizedAt = args.status === "failed" || args.status === "cancelled"
            ? Date.now()
            : undefined;
...
// Ensure there is a worker running to grab batches of emails.
async function scheduleBatchRun(ctx, options) {
    ...
    const runId = await ctx.scheduler.runAfter(BASE_BATCH_DELAY, internal.lib.makeBatch, { reloop: false, segment: getSegment(Date.now() + BASE_BATCH_DELAY) });
...
export const makeBatch = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        await emailPool.enqueueAction(ctx, internal.lib.callResendAPIWithBatch, {
...
async function reschedule(ctx, emailsLeft) {
    emailsLeft =
        emailsLeft ||
            (await ctx.db
                .query("emails")
...
export const callResendAPIWithBatch = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        if (!response.ok) {
            ...
            return {
                emailIds,
                resendIds: data.data.map((d) => d.id),
            };
        }
    },
});
...
async function markEmailsFailedHandler(ctx, args) {
    await Promise.all(args.emailIds.map(async (emailId) => {
...
export const onEmailComplete = emailPool.defineOnComplete({
    ...
    handler: async (ctx, args) => {
        if (args.result.kind === "success") {
            ...
            await Promise.all(emailIds.map((emailId, i) => ctx.db.patch(emailId, {
                status: "sent",
            ...
            await Promise.all(args.context.emailIds.map(async (emailId) => {
                const email = await ctx.db.get(emailId);
                if (!email || email.status !== "queued") {
                    return;
                }
...
async function createResendBatchPayload(ctx, emailIds) {
    ...
    const contentMap = await getAllContent(ctx, emails
    ...
    const batchPayload = emails.map((email) => ({
        ...
        headers: email.headers
            ? Object.fromEntries(email.headers.map((h) => [
                h.name,
                h.value,
            ]))
            : undefined,
...
export const getAllContentByIds = internalQuery({
    ...
    handler: async (ctx, args) => {
        ...
        for (const doc of docs) {
            if (!doc)
                throw new Error("Content not found -- invariant");
            contentMap.push({
                id: doc._id,
...
export const getEmailByResendId = internalQuery({
    ...
    handler: async (ctx, args) => {
        const email = await ctx.db
            ...
            .unique();
        if (!email)
            throw new Error("Email not found for resendId");
        return email;
    },
});
...
export const handleEmailEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (result.kind === "error") {
            console.warn(`Invalid email event received. You might want to to exclude this event from your Resend webhook settings in the Resend dashboard. ${result.error}.`);
            return;
        }
        ...
        const email = await ctx.db
            ...
            .unique();
        if (!email) {
            console.info(`Email not found for resendId: ${event.data.email_id}, ignoring...`);
            return;
        }
        ...
        if (changed)
            await ctx.db.replace(email._id, changed);
        await enqueueCallbackIfExists(ctx, changed ?? email, event);
    },
});
async function enqueueCallbackIfExists(ctx, email, event) {
    const lastOptions = await ctx.db.query("lastOptions").unique();
    if (!lastOptions) {
        throw new Error("No last options found -- invariant");
    }
    if (lastOptions.options.onEmailEvent) {
        const handle = lastOptions.options.onEmailEvent.fnHandle;
...
export const cleanupOldEmails = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const oldAndDone = await ctx.db
            .query("emails")
            .withIndex("by_finalizedAt", (q) => q.lt("finalizedAt", Date.now() - olderThan))
            .take(500);
...
export const cleanupAbandonedEmails = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const oldAndAbandoned = await ctx.db
            .query("emails")
            .withIndex("by_creation_time", (q) => q.lt("_creationTime", Date.now() - olderThan))
            .take(500);
